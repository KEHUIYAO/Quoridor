...
class Quoridor(object):
    ...
    def _num_of_steps_to_goal(self, intersections, target_row, player_position, opponent_position, player=1):
        visited = []
        invalid_rows = [9, -1]
        visit_queue = Queue()
        visit_queue.put([player_position,0])
        target_visited = False

        while not target_visited and not visit_queue.empty():
            current_position = visit_queue.get()
            valid_directions = self._valid_pawn_actions(intersections,
                                                        location=current_position[0],
                                                        opponent_loc=opponent_position,
                                                        player=player)
            ...
            for direction in valid_directions:
                if direction == self._DIRECTIONS['N']:
                    new_position = [current_position[0] + 9, current_position[1] + 1]
                elif direction == self._DIRECTIONS['S']:
                    new_position = [current_position[0] - 9, current_position[1] + 1]
                elif direction == self._DIRECTIONS['E']:
                    ...
                    new_position = [current_position[0] + 18, current_position[1] + 1]
                elif direction == self._DIRECTIONS['SS']:
                    new_position = [current_position[0] - 18, current_position[1] + 1]
                elif direction == self._DIRECTIONS['EE']:
                    new_position = [current_position[0] + 2, current_position[1] + 1]
                elif direction == self._DIRECTIONS['WW']:
                    new_position = [current_position[0] - 2, current_position[1] + 1]
                elif direction == self._DIRECTIONS['NE']:
                    new_position = [current_position[0] + 10, current_position[1] + 1]
                elif direction == self._DIRECTIONS['NW']:
                    new_position = [current_position[0] + 8, current_position[1] + 1]
                elif direction == self._DIRECTIONS['SW']:
                    new_position = [current_position[0] - 10, current_position[1] + 1]
                elif direction == self._DIRECTIONS['SE']:
                    new_position = [current_position[0] - 8, current_position[1] + 1]
                ...
                new_row = new_position[0] // self.N_ROWS
                if new_row == target_row:
                    target_visited = True
                elif new_position[0] not in [i[0] for i in visited]:
                    visited.append(new_position)
                    if new_row not in invalid_rows:
                        visit_queue.put(new_position)
    ...
    def start_self_play(self, player, is_shown=0, temp=1e-3):
        ...
        while(1):   # 循环进行自博弈
            ...
            if self._player1_walls_remaining == 0 and self._player2_walls_remaining == 0:
                ...
                player1_num_of_steps_to_win = self._num_of_steps_to_goal(intersections, player1_target, player1_position, player2_position,
                                                  player=1)
                player2_num_of_steps_to_win = self._num_of_steps_to_goal(intersections, player1_target, player1_position, player2_position,
                                                  player=2)




                if player1_num_of_steps_to_win < player2_num_of_steps_to_win:
                    winner = 1
